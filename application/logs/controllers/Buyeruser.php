   ...
   // exit;
    switch ($value_details) {
        ...
        case '005110-1': // here first technical  is been complted but commercial is pendin send to buyer to set information is about Commerical
            # code...
            break;
        case '005200-1': // here first technical  is been complted but commercial is pendin send to buyer to set information is about Commerical
            # code...
            break;
        //Moi
   ...
   public function buyer_file_upload_data($value=''){
    ...
    public function buyer_bid_tech_entry($value=''){
        ...
        if(!empty($vendor_array)){
            $data_bid_master = array('buyer_slno'=>$value4, 'bid_date_entry'=>$date_create, 'bid_ref'=>$bid_ref_no, 'bid_id'=>$bid_Id, 'category'=>$materials_id, 'mode_bid'=>$bid_method, 'technical_bid_type'=>$tech_evalution, 'status_bid'=>'4', 'mr_slno'=>$slno_Mr_no, 'mr_no'=>$mr_no, 'job_code'=>$job_code_id, 'edit_id'=>$edit_id, 'material_category_name'=>$category_name, 'bid_title'=>$bid_title, 'bid_description'=>$bid_work_description);
            ...
            if($query_bid_master){
                ...
                $data_bid_date = array('bid_slno'=>$slno_Mr_no, 'buyer_slno'=>$value4, 'bid_start_date'=>$date_start_bid, 'bid_closed_date'=>$date_closed_bid, 'bid_query_closed_date'=>$date_clearfication_bid, 'status'=>'4', 'master_bid_id'=>$insert_id);
                ...
                foreach ($vendor_array as $key_vendor) {
                    $vendor_infom = array('bid_slno'=>$slno_Mr_no, 'buyer_slno'=>$value4, 'vendor_id'=>$key_vendor, 'status'=>4, 'title'=>$bid_title, 'description'=>$bid_work_description, 'date_start'=>$date_start_bid, 'date_end'=>$date_closed_bid, 'query_end date'=>$date_clearfication_bid, 'master_bid_id'=>$insert_id,'bid_ref'=>$bid_ref_no, 'bid_id'=>$bid_Id, 'category'=>$materials_id, 'mode_bid'=>$bid_method, 'technical_bid_type'=>$tech_evalution, 'mr_slno'=>$slno_Mr_no, 'mr_no'=>$mr_no, 'job_code'=>$job_code_id, 'edit_id'=>$edit_id, 'material_category_name'=>$category_name );
             ...
             $this->session->set_flashdata('error_message',  'Novendor is Selletec');
                redirect('user-buyer-home');
        }
