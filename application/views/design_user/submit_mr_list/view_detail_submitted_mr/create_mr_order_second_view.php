				...
				<div class="panel-body">
					...
					<form action="#" id="upload_form" method="POST" enctype=multipart/form-data>
						<div class="row">
						 ...
						 <!-- <div class="row">
						 	<div class="col-md-12 col-lg-12">
								 ...
								 $result_item=$this->design_user->get_design_master_items_material(2);
						...
						<div class="form-group row pull-right">
                            <div class="col-md-12">
                                <?php echo '<a href="'.base_url().'design-submitted-mr-order-detail/'.$job_code_id.'/2" class="btn btn-lime btn-sm">Back</a>'; ?>
                                <a  href="<?=base_url()?>user-design-home" class="btn btn-sm btn-info">Home</a>
                            </div>
                        </div>
...
var queryString = "";
	if(action != "") {
		switch(action) {
			case "add":
				queryString = 'action='+action+'&code='+ item_codes_order+'&quantity='+$("#qty_"+item_codes_order).val();
...
function get_value_check(id,value_id) {
	...
	if (checkBox.checked == true){
		...
		// alert(get_detail);
		queryString_id = 'action='+action_id+'&code='+ get_detail;
 		// alert(get_detail);
 		jQuery.ajax({
			url: "<?=base_url()?>item_required_session",
			data:queryString_id,
