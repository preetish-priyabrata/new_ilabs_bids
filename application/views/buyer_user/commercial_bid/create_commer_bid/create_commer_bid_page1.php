...
<div class="sidebar-bg"></div>
	...
	<!-- begin breadcrumb -->
		<ol class="breadcrumb pull-right">
			...
			<li class="breadcrumb-item active">Commercial Details</li>
		</ol>
		<!-- end breadcrumb -->
		<!-- begin page-header -->
		<h1 class="page-header"><small>Buyer Commercial Setting</small></h1>
		<!-- end page-header -->
	<?php
	...
	<div class="panel panel-inverse">
		<div class="panel-heading">
			...
			<h4 class="panel-title">Create Commercial REF</h4>
		</div>
		<div class="panel-body">
			<div class="alert alert-secondary">
                <span style="color: red"> *</span> All mandatory fields shall be duly filled up
            </div>
			<form action="<?=base_url()?>bid-commerical-entry" method="POST"  id="bid_tech" name="bid_tech" class="bid_tech">
					                                ...
					                                <tbody>
					                                	...
					                                	// echo "<pre>";
					                                		if($query_item_details_list['no_item']==1){
					                                			...
					                                			?>
					                                			<input type="hidden" name="item_name[]" value="<?=$query_item_details['materials_list'][0]->item_name?>">
					                                			<input type="hidden" name="item_id[]" value="<?=$query_item_details['materials_list'][0]->item_id?>">
					                                			<input type="hidden" name="item_uom[]" value="<?=$query_item_details['materials_list'][0]->item_uom?>">
					                                			<input type="hidden" name="material_quantity[]" value="<?=$material_quantity?>">
					                                			<input type="hidden" name="slno_item_mr[]" value="<?=$slno_item_mr?>">
					                                			<tr>
					                                				<td><?=$query_item_details['materials_list'][0]->item_name?></td>
					                                				<td><?=$item_id=$query_item_details['materials_list'][0]->item_id?></td>
							                                       	...
							                                       	<td>
							                                       		...
							                                       		if($get_technical['no_received']==1){
							                                       			...
							                                       			foreach ($procuremenr_list_single as $key_value_technical) {
							                                       				echo $tech_name=$key_value_technical->tech_name;
							                                       				echo '<br><input type="hidden" name="tech_name['.$item_id.'][]" value="'.$tech_name.'">';
							                                       			}
							                                       		}else{
							                                       			echo 'No Parameter is been Selected <input type="hidden" name="tech_name['.$item_id.'][0]" value="No Parameter is been Selected">';
							                                       		}
							                                       		?>
							                                       	</td>
							                                       	<td><input type="text" name="unit_price[<?=$item_id?>]" required></td>
							                                    </tr>
					                                		<?php
					                                			}
					                                		}
